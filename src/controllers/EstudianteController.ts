...
export const validar = () => [
    check('dni')
        ...
        .isLength({ min: 7 }).withMessage('El DNI debe tener al menos 7 numeros'),
...
export const consultarTodos = async (req: Request, res: Response) => {
    try {
        ...
        console.log(Array.isArray(estudiantes));
...
export const consultarEstudiante = async (req: Request, res: Response)=> {
    ...
    try {
        ...
        const estudianteRepository = AppDataSource.getRepository(Estudiante);
        estudiantes = await estudianteRepository.find({ where: { id: estudianteId } })
        console.log(`Buscando estudiante con ID: ${id}`); // Asegúrate de que esto muestra el ID correcto.
